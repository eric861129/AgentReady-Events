...
- 公開 `/health/version` 與這組 commit／revision 一致，因此 runtime 可列為 E3；完整自動化與 production smoke 已通過，targeted recovery v2 的 Inspector 重測尚未開始。
...
- revision `0000009` 使用獨立的 `targeted-recovery-v2.json`；在逐題 Inspector trace 完成前，不主張新的 E4 或成功率。
